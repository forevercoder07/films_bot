...
# Canceling states on other admin buttons
ADMIN_BTN_SET = ["Admin", "Channels", "Delete film", "Add film part", "User Statistic", "Film Statistic", "All write", "Add admin"]
@dp.message(AddFilm.code, F.text.in_(ADMIN_BTN_SET))
...
@dp.message(AddFilm.title, F.text.in_(ADMIN_BTN_SET))
...
async def admin_add_film_part_video(msg: Message, state: FSMContext):
    ...
    if msg.video:
        ...
        video_id = (msg.text or "").strip()

    await add_part(film_code=code, title=title, description="", video=video_id)
    await msg.answer("Qism qo‘shildi ✅")
    await state.clear()

@dp.message(AddPart.movie_code, F.text.in_(ADMIN_BTN_SET))
@dp.message(AddPart.title, F.text.in_(ADMIN_BTN_SET))
@dp.message(AddPart.video, F.text.in_(ADMIN_BTN_SET))
...
# -------------------------
# Admins list (fix: get_all_admins returns list[int])
# -------------------------
...
async def admin_list(msg: Message):
    ...
    for idx, adm_id in enumerate(admins, start=1):
        buttons.append([
            InlineKeyboardButton(text=f"{idx}-admin", url=f"tg://user?id={adm_id}")
        ])
...
async def admin_add_admin_perms(msg: Message, state: FSMContext):
    data = await state.get_data()
    aid = data["admin_id"]
    raw = [s.strip() for s in (msg.text or "").split(",")]
...
async def _debug_all_messages(msg: Message):
    logger.info(
        "DEBUG MSG from %s username=%s text=%r",
        msg.from_user.id,
